---------------------------------------------------------------------------------------------------
-- User story: https://github.com/smartdevicelink/sdl_core/issues/2556
-- Description:
...
-- 1) 
-- Expected result:
-- 1) SDL processes json as valid (according to comment in #994)
-- Actual result:
-- 1) SDL processes json as not valid
---------------------------------------------------------------------------------------------------
...
local function RegisterAppWithOnSystemRequestQueryApps()
    local pAppId = 1
    ...
    :Do(function(_, data)
        if data.payload.requestType == "QUERY_APPS" then
            local CorIdSystemRequest = common.getMobileSession(pAppId):SendRPC("SystemRequest",{
                requestType = "QUERY_APPS",
                fileName = "JSONWithTtsNameVrSynonymsInSeparatedStructs.json"
            },
            "files/jsons/JSON_Language_parameter/JSONWithTtsNameVrSynonymsInSeparatedStructs.json")
...
runner.Title("Test")
runner.Step("Register App with OnSystemRequest Query_Apps", RegisterAppWithOnSystemRequestQueryApps)
